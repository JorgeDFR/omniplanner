...
from dsg_pddl.pddl_utils import lisp_string_to_ast, extract_facts
...
def extract_symbols_of_interest(pddl_goal, containment_relations):
    ...
    for cont_type, symbol_1, symbol_2 in containment_relations:
        if cont_type == "object-in-place" and symbol_1 in [obj.symbol for obj in object_symbols]:
    ...
    for cont_type, symbol_1, symbol_2 in containment_relations:
        ...
        if cont_type == "place-in-region" and symbol_1 in [place.symbol for place in place_symbols]:
            containment_relations_of_interest.append((cont_type, symbol_1, symbol_2))
...
def main():
    ...
    places, objects, containment_relations_of_interest = extract_symbols_of_interest(goal, containment_relations)
    all_places, _, regions = retrive_dsg_symbols(G)

    pstart = PddlSymbol("pstart", "place", ["at-poi"], position=[0.0, 0.0])
    places.append(pstart)
    all_places.append(pstart)
    init = [('place', p.symbol.lower()) for p in places] + \
    ...
    def sample_region(region):
        while True:
            for cont_type, symbol_1, symbol_2 in containment_relations:
                if cont_type == "place-in-region" and symbol_2 == region \
    ...
    solution = solve(
        ...
        unit_costs=True,
        planner='ff-astar2', # 'ff-astar' | 'ff-astar2' | https://www.fast-downward.org/latest/documentation/search/SearchAlgorithm/
        max_time=20.0,
